...
import { X, CalendarPlus, CalendarCheck, Trash2, CheckCircle, DollarSign, AlertCircle, Clock, Filter, ChevronDown, Link as LinkIcon, Edit, Save } from 'lucide-react';
...
export default function StudentDetailModal({ isOpen, onClose, studentId, onOpenAddAula, onOpenAddAulaLote, showToast }) {
  ...
  useEffect(() => {
    if (isOpen && studentId && teacherData && teacherData.students) {
      ...
      if (!foundStudent) {
        console.warn("StudentDetailModal: Aluno com ID", studentId, "não encontrado.");
      }
      setIsEditMode(false); 
  ...
  const handleEditClick = () => {
    ...
    setEditedData({
      ...
      lessonValue: student.lessonValue || '',
  ...
  const handleSaveStudent = async () => {
    ...
    const lessonValueNum = parseFloat(editedData.lessonValue);
    const paymentDayNum = parseInt(editedData.paymentDay, 10);

    const studentPayload = {
      name: editedData.name.trim(),
      studentEmail: editedData.studentEmail.trim() || null,
      lessonLink: editedData.lessonLink.trim() || null,
      lessonValue: isNaN(lessonValueNum) ? 0 : lessonValueNum,
      paymentDay: isNaN(paymentDayNum) ? null : paymentDayNum,
    };

    try {
      await updateStudentInDb(currentUser.uid, student.id, studentPayload);
      if(showToast) showToast(`Aluno "${studentPayload.name}" atualizado com sucesso!`, 'success');
      setIsEditMode(false);
    } catch (error) {
      console.error("Erro ao atualizar aluno:", error);
      if(showToast) showToast(error.message || "Erro ao atualizar aluno.", "error");
    } finally {
  ...
  const inputStyle = `w-full shadow-none text-gray-800 rounded-md p-1 ${isEditMode ? 'bg-indigo-50 border border-indigo-300 focus:ring-2 focus:ring-indigo-500' : 'bg-transparent border-none focus:ring-0 cursor-default'}`;
      ...
      <div className="relative mx-auto border border-gray-200 w-full max-w-4xl shadow-xl rounded-lg bg-white">
        <div className="p-6 max-h-[90vh] overflow-y-auto">
          <div className="flex justify-between items-center border-b border-gray-200 pb-3 mb-6">
            {isEditMode ? (
                <input
                    ...
                    className="text-2xl leading-6 font-semibold text-gray-900 bg-indigo-50 border border-indigo-300 rounded-md p-1 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                    disabled={isSubmitting}
                />
            ) : (
                <h3 className="text-2xl leading-6 font-semibold text-gray-900">{student.name}</h3>
            )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4 mb-6 text-sm">
            <div>
              <label className="font-medium text-gray-500 block mb-1">Email do Aluno:</label>
              <input type="email" name="studentEmail" value={isEditMode ? editedData.studentEmail : student.studentEmail || ''} onChange={handleInputChange} disabled={!isEditMode || isSubmitting} className={inputStyle} />
            </div>
            <div>
              <label className="font-medium text-gray-500 block mb-1">Link Padrão da Aula:</label>
              <input type="url" name="lessonLink" value={isEditMode ? editedData.lessonLink : student.lessonLink || ''} onChange={handleInputChange} disabled={!isEditMode || isSubmitting} className={inputStyle} />
            </div>
            <div>
              <label className="font-medium text-gray-500 block mb-1">Valor por Aula (R$):</label>
              <input type="number" name="lessonValue" value={isEditMode ? editedData.lessonValue : student.lessonValue || ''} onChange={handleInputChange} disabled={!isEditMode || isSubmitting} className={inputStyle} step="0.01" min="0" />
            </div>
            <div>
              <label className="font-medium text-gray-500 block mb-1">Dia de Pagamento:</label>
              <input type="number" name="paymentDay" value={isEditMode ? editedData.paymentDay : student.paymentDay || ''} onChange={handleInputChange} disabled={!isEditMode || isSubmitting} className={inputStyle} min="1" max="31" />
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2 flex-wrap gap-2 mb-6 border-b border-gray-200 pb-6">
            {isEditMode ? (
              <>
                <button onClick={handleSaveStudent} disabled={isSubmitting} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50">
                    {isSubmitting ? 'A salvar...' : <><Save className="h-5 w-5 mr-2" /> Salvar</>}
                </button>
                <button onClick={handleCancelEdit} disabled={isSubmitting} className="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 disabled:opacity-50">
                    Cancelar
                </button>
              </>
            ) : (
              <>
                <button onClick={handleEditClick} className="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" title="Editar Aluno"><Edit className="h-5 w-5 mr-2" /> Editar</button>
                <button onClick={() => onOpenAddAula(student.id)} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700" title="Adicionar Aula Única"><CalendarPlus className="h-5 w-5 mr-2" /> Add Aula</button>
                <button onClick={() => onOpenAddAulaLote(student.id)} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700" title="Cadastrar Aulas em Lote"><CalendarCheck className="h-5 w-5 mr-2" /> Em Lote</button>
